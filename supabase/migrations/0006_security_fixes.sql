-- ============================================================================
-- STEPS SEGUROS · Fixes de seguridad (auditoría)
-- Migración 0006
-- M1: quita el backstop de IP dentro de submit_quote_request (throttleaba el
--     tráfico legítimo; redundante con la firma HMAC + rate limit de la capa Next).
-- M2: RPC update_tenant_branding para que tenant_admin guarde su config
--     (RLS solo deja escribir tenants a super_admin). Dominio/subdominio quedan
--     restringidos a super_admin a nivel de columna.
-- ============================================================================
...
declare
  ...
  v_secret text; v_expected text; v_now bigint;
begin
...
declare v_is_super boolean; v_authorized boolean;
begin
  v_is_super := public.is_super_admin();
  v_authorized := public.has_tenant_role(p_tenant_id, array['tenant_admin']::public.app_role[]);
  if not v_authorized then
    raise exception 'No autorizado para editar este tenant';
  end if;
  if p_color_primary !~ '^#[0-9A-Fa-f]{6}$'
     or p_color_secondary !~ '^#[0-9A-Fa-f]{6}$'
     or p_color_accent !~ '^#[0-9A-Fa-f]{6}$' then
    raise exception 'Color inválido';
  end if;

  update public.tenants set
    nombre_comercial = p_nombre_comercial,
    whatsapp_number  = nullif(p_whatsapp_number, ''),
    contact_email    = nullif(p_contact_email, ''),
    color_primary    = p_color_primary,
    color_secondary  = p_color_secondary,
    color_accent     = p_color_accent,
    is_active        = p_is_active,
    subdomain     = case when v_is_super then nullif(p_subdomain, '') else subdomain end,
    custom_domain = case when v_is_super then nullif(p_custom_domain, '') else custom_domain end,
    updated_at    = now()
  where id = p_tenant_id;
end;
$$;
revoke all on function public.update_tenant_branding(uuid, text, text, text, text, text, text, boolean, text, text) from public, anon;
grant execute on function public.update_tenant_branding(uuid, text, text, text, text, text, text, boolean, text, text) to authenticated;
