...
export default async function PortalLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItemClass =
    'inline-flex shrink-0 items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium text-navy-600 hover:bg-brand-primary/5 hover:text-brand-primary'

  return (
    <div className="min-h-screen bg-mist">
      <header className="sticky top-0 z-40 border-b border-steel bg-white">
        <Container className="flex h-16 items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <form action={logoutAction}>
              <button
                type="submit"
                className="inline-flex items-center gap-2 rounded-lg border border-steel px-3 py-2 text-sm font-medium text-navy-600 hover:bg-mist"
              >
        ...
        <nav className="flex gap-1 overflow-x-auto border-t border-steel px-3 py-2 sm:hidden">
          {navItems.map((i) => (
            <Link key={i.href} href={i.href} className={navItemClass}>
              <i.icon className="h-4 w-4" /> {i.label}
            </Link>
