...
export default async function QuoteDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-4xl">
      <Link href="/portal" className="inline-flex items-center gap-1 text-sm text-navy-600 hover:text-brand-primary">
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[1.5fr_1fr]">
        ...
        <div className="space-y-6">
          <section className="rounded-2xl border border-steel bg-white p-6">
            <h2 className="text-sm font-semibold uppercase tracking-wide text-navy-400">Datos del cliente</h2>
          ...
          <section className="rounded-2xl border border-steel bg-white p-6">
            <h2 className="text-sm font-semibold uppercase tracking-wide text-navy-400">Sobre el riesgo</h2>
            <div className="mt-4 space-y-3">
              ...
              {typeof risk.descripcion === 'string' && risk.descripcion && (
                <div>
                  <p className="text-xs font-medium text-navy-400">Descripción</p>
                  <p className="mt-1 whitespace-pre-wrap text-sm text-navy-700">{risk.descripcion}</p>
                </div>
              )}
        ...
        <aside className="space-y-6">
          ...
          <section className="rounded-2xl border border-steel bg-white p-6">
            <h2 className="text-sm font-semibold uppercase tracking-wide text-navy-400">Gestión de cotización</h2>
            <p className="mt-3 text-sm text-navy-600">
              Proveedor: <span className="font-medium">{quote.provider ?? 'manual'}</span>
            </p>
            <p className="mt-2 rounded-lg bg-brand-primary/5 p-3 text-xs text-navy-600">
